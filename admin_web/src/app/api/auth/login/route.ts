...
import { login, LoginError } from "@/server/auth/application/commands/login";
...
export async function POST(request: Request) {
  ...
  try {
    const result = await login(container, body);
    ...
    if (err instanceof LoginError) {
      const status =
        err.code === "INVALID_CREDENTIALS" ? 401 : err.code === "USER_DISABLED" ? 403 : 409;
